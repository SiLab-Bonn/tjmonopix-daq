...
class TJMonoPix(Dut):
    ...
    def __init__(self, conf=None, **kwargs):
        ...
        self.conf_flg=1 
        self.SET={'VDDA':None, 'VDDP':None, 'VDDA_DAC':None, 'VDDD':None, 'VPCSWSF':None, 'VPC':None, 'BiasSF':None,'INJ_LO':None, 'INJ_HI':None}
    ...
    def init(self,B=True):
        ...
        self.power_on()
        self.set_monoread(en=False)  ## just start readout, data will not be recorded        
        self.default_conf()
        self.write_conf()
    ...
    def power_on(self, VDDA=1.8,VDDP=1.8, VDDA_DAC=1.8, VDDD=1.8, VPCSWSF=0.5, VPC=1.3, BiasSF=100E-6,INJ_LO=0.2, INJ_HI=3.6):
        # Set power
        ...
        self.SET["VPC"]=VPC
        self['BiasSF'].set_current(BiasSF*1E-6, unit='uA')
    ...
    def set_all_mask(self,mask=None):
        ...
        if mask==None:
            ...
            self['CONF_SR']['EN_HITOR_OUT'].setall(True) #ENABLES OR DISABLES THE NORMAL HITOR PADS, HITOR0-3 =  1-4 flavor, ACTIVE LOW
            ...
            self['CONF_SR']['EN_OUT'].setall(True) #ENABLES OR DISABLES THE NORMAL OUTPUT PADS, ACTIVE LOW
            self['CONF_SR']['nEN_OUT'].setall(True) #ENABLES OR DISABLES THE COMPLEMENTARY OUTPUT PADS, ACTIVE LOW
            # injection
...
############################## SET BIAS CURRENTS AND VOLTAGES ##############################
    ...
    def set_ibias_dacunits(self, dacunits, printen=True):
        ...
        if printen:
            logging.info("set_ibias_dacunits: ibias=%.4fnA dac=%d,%d-%d"%(1400.0*((dacunits+1)/128.0),dacunits,high,low))
    ...
    def set_idb_dacunits(self, dacunits, printen=True):
        ...
        if printen:
            logging.info("set_idb_dacunits: idb=%.4fnA dac=%d,%d-%d"%(2240.0*((dacunits+1)/128.0),dacunits,high,low))
    ...
    def set_ithr_dacunits(self, dacunits, printen=True):
        ...
        if printen:
            logging.info("set_ithr_dacunits: ithr=%.4fnA dac=%d,%d-%d"%(17.5*((dacunits+1)/128.0),dacunits,high,low))
    ...
    def set_icasn_dacunits(self, dacunits, printen=True):
        ...
        if printen:
            logging.info("set_icasn_dacunits: icasn=%.4fnA dac=%d,%d-%d"%(560.0*((dacunits+1)/128.0),dacunits,high,low))
    ...
    def set_ireset_dacunits(self, dacunits, printen=True):
        ...
        if printen:
            ...
            logging.info("set_icasset_ireset_dacunits: icasn=%.4fpA dac=%d,%d,%d-%d"%(560.0*((dacunits+1)/128.0),dacunits,mode,high,low))
    ...
    def set_vreset_dacunits(self, dacunits, printen=True):
        ...
        if printen:
            logging.info("set_vcasn_dac_dacunits: vreset=%.4fV dac=%d"%((1.8/127.0)*dacunits+0.555,dacunits))
    ...
    def set_vh_dacunits(self, dacunits, printen=True):
        ...
        if printen:
                logging.info("set_vh_dacunits: vh=%.4fV, dac=%d"%((1.8/127.0)*dacunits+0.385,dacunits))
    ...
    def set_vl_dacunits(self, dacunits, printen=True):
        ...
        if printen:
            logging.info("set_vl_dacunits: vl=%.4fV dac=%d"%((1.8/127.0)*dacunits+0.385,dacunits))
    ...
    def set_vcasn_dac_dacunits(self, dacunits, printen=True):
        ...
        if printen:
            logging.info("set_vcasn_dac_dacunits: vcasn=%.4fV dac=%d"%((1.8/127.0)*dacunits,dacunits))
