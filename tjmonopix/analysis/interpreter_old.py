...
def list2img(dat, delete_noise=True):
    ...
    return np.histogram2d(dat["col"], dat["row"], bins=[np.arange(0, 112 + 1, 1), np.arange(0, 224 + 1, 1)])[0]
...
def without_noise(dat):
    # TODO: Seems fishy, should be dat["col"] < COL
    return dat[np.bitwise_or(dat["cnt"] == 0, dat["col"] >= 112)]
...
class Interpreter():
    ...
    def run(self, raw, data_format=0x43):
        ...
        ret = np.empty(0, dtype=self.hit_dtype)
    ...
    def mk_list(self, raw, delete_noise=True):
        dat = self.run(raw)
        if delete_noise is True:
            dat = without_noise(dat)
        return dat

    def mk_img(self, raw, delete_noise=True):
        dat = self.run(raw)
        return list2img(dat, delete_noise)

    def mk_cnt(self, raw, delete_noise=True):
        dat = self.run(raw)
        return list2cnt(dat, delete_noise)
...
def raw2img(raw, delete_noise=True):
    inter = Interpreter()
    return list2img(inter.run(raw), delete_noise)


def raw2cnt(raw, delete_noise=True):
    inter = Interpreter()
    return list2cnt(inter.run(raw), delete_noise)
